...
root    = '/projectnb/cui-buchem/tanmoy/projects/ionic_liquids/400k/1_emim_ace/tcf/'
drf     =   root + 'field_time'
wrk     =   os.getcwd()
r_cut   =   3.5
...
def calc_field(refIndex, noGold, boxDim, chargeList, rcut):
    ...
    fProjCN		= np.dot(fTotNoIntra, projVecNorm) 
    return(fIntra, fLigand, fCation, fAnion, fTotNoIntra, fProjCN)

#--get coordinates as gro file--
def gen_trajout(time):
    gmx_trj_cmd = "echo 0 | gmx trjconv -f " + root + "prod_tcf.xtc -s " + root + "prod_tcf.tpr"\
...
simTime        = _GRPNO_ + 0.01 * _TNO_ 
gen_trajout(simTime)

#--process gro file--
tstamp, n_atoms, boxInfo, molinfo = process_gro(wrk + '/trajout.gro')
idx_ref      = get_ref_idx( drf + '/index_nz_tcf.ndx')
...
f06		= open("field_projection_cn.dat", "w")
#--read charges from charges

for index in idx_ref:
    fIntra, fLigand, fCation, fAnion, fTotNoIntra, fProjCN = calc_field(index, noGold, boxInfo, chargeList, r_cut)
